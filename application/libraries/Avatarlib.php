...
class Avatarlib {
	function __construct() {
		 ...
		 *上传响应程序URL
		 */
		$this->doaction = $this->base_url.'api/avatar/doavatar';
		/**
		 ...
		 * 保存图片目录,777属性哦
		 */
		$this->avatarpath =  'assets/'.APPLICATION.'/avatar/';
		/**
		 * FCPATH CI你懂的 
		 */
	...
	function uc_avatar($uid, $returnhtml = TRUE) {
		...
		$uc_avatarflash = $this->base_url . '/themes/default/images/camera.swf?inajax=1&appid=1&input=' . $input . '&agent=' . md5 ( $_SERVER ['HTTP_USER_AGENT'] ) . '&ucapi=' . urlencode ( $this->doaction ) . '&avatartype=real';
		if ($returnhtml) {
	...
	function onrectavatar() {
		...
		if (! $bigavatar || ! $middleavatar || ! $smallavatar) {
			return '<root><message type="error" value="-2" />'.$bigavatarfile.'</root>';
		}
		$success = 1;
	...
	function avatar_show($uid, $size = 'small', $returnsrc = FALSE) {
		...
		if($returnsrc){
			return $this->base_url.(file_exists($this->systempath.$this->avatarpath . $avatarfile)?($this->avatarpath . $avatarfile):( 'images/'.APPLICATION.'/noavatar_' . $size . '.gif'));
		}
		return '<img src="' . $this->base_url . $this->avatarpath . $avatarfile . '" onerror="this.onerror=null;this.src=\'' . $this->base_url . 'images/'.APPLICATION.'/noavatar_' . $size . '.gif\'">';
	}
