...
class Authcheckcode {
	...
	var $showBorder = FALSE; //边框，当杂点、线一起作用的时候，边框容易受干扰
	var $borderColor = "#000000";
	
	function __construct() {
		$this->ci = & get_instance ();
		$this->ci->load->library ( 'session' );
		$this->fontPath = APPPATH . 'fonts/'; //字体文件
	...
	function show() {
		...
		$this->ci->session->set_userdata ( 'authcode', $code );
		ob_clean ();
	...
	function check($auth_code = null) {
		return ($this->ci->session->userdata ( 'authcode' ) && $auth_code) ? (strtolower ( $this->ci->session->userdata ( 'authcode' ) ) == strtolower ( $auth_code )) : false;
	}
